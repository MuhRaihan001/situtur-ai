        ...
        <nav className="flex flex-col items-start px-4 mt-6 gap-1 flex-1">
          {menuItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`flex flex-row items-center w-full h-[44px] px-[12px] py-[8px] gap-[12px] rounded-[8px] transition-colors ${isActive(item.path)
        ...
        <div className="p-4 border-t border-[#E5E7EB] bg-white space-y-1">
          ...
          <div className="pt-2">
            <button
              ...
              className="flex flex-row justify-center items-center w-full h-10 px-3 py-2 gap-2 bg-[#0F172A] rounded-lg hover:bg-[#1e293b] transition-colors group"
            >
